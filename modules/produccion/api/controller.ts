...
export const registrarProduccion = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (turno && !TURNOS.includes(turno)) return res.status(400).json({ error: 'Turno inválido' });
    ...
    if (!plato || plato.estado !== 'Activo') return res.status(404).json({ error: 'Plato no encontrado o inactivo' });
    if (plato.receta.length === 0) return res.status(400).json({ error: 'El plato no tiene receta cargada' });
    ...
    const lotesPorInsumo = new Map<number, Awaited<ReturnType<typeof prisma.inventario.findMany>>>();
    const faltantes = [];

    for (const reqItem of requerimientos) {
      const lotes = await prisma.inventario.findMany({
        where: {
