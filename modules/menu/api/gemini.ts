...
export interface GeminiPlatoSuggestion {
  descripcion: string;
  receta: GeminiRecipeItem[];
  fuente: 'gemini';
}

export interface GeminiRecipeSteps {
  titulo: string;
  rendimiento: string;
...
const buildPrompt = (nombre: string, insumos: CatalogInsumo[], compact = false) => [
...
const parseSuggestion = (rawText: string, insumos: CatalogInsumo[]): GeminiPlatoSuggestion | null => {
  const parsed = extractJson(rawText);
  if (!parsed || typeof parsed.descripcion !== 'string' || !Array.isArray(parsed.receta)) return null;

  const insumoById = new Map(insumos.map(insumo => [insumo.id, insumo]));
  const used = new Set<number>();
  const receta: GeminiRecipeItem[] = [];
  ...
  for (const item of parsed.receta) {
    ...
    if (!insumo || used.has(insumoId) || !Number.isFinite(cantidad) || cantidad <= 0) continue;
...
const parseRecipeSteps = (rawText: string): GeminiRecipeSteps | null => {
  const parsed = extractJson(rawText);
  if (!parsed || !Array.isArray(parsed.pasos)) return null;

  const pasos = parsed.pasos
    .map((paso: unknown) => String(paso || '').trim())
    .filter(Boolean)
    .slice(0, 12);
  ...
  return {
    ...
    tips: Array.isArray(parsed.tips)
      ? parsed.tips.map((tip: unknown) => String(tip || '').trim()).filter(Boolean).slice(0, 5)
      : [],
    fuente: 'gemini'
  };
};

export const suggestPlatoWithGemini = async (nombre: string, insumos: CatalogInsumo[]): Promise<GeminiPlatoSuggestion> => {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) throw new Error('GEMINI_API_KEY no configurada');
  if (insumos.length === 0) throw new Error('No hay insumos activos para sugerir receta');

  const model = process.env.GEMINI_MODEL || 'gemini-flash-latest';
  const requestSuggestion = (prompt: string) => fetch(`${GEMINI_ENDPOINT}/${model}:generateContent`, {
    method: 'POST',
    headers: {
...
export const generateRecipeStepsWithGemini = async (input: RecipeStepInput): Promise<GeminiRecipeSteps> => {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) throw new Error('GEMINI_API_KEY no configurada');
