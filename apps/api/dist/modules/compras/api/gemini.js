...
exports.explainPurchaseSuggestionWithGemini = void 0;
const GEMINI_ENDPOINT = 'https://generativelanguage.googleapis.com/v1beta/models';
const extractReasons = (text) => {
    const reasons = new Map();
    for (const line of text.split('\n')) {
        const match = line.match(/^(\d+)\s*:\s*(.+)$/);
        if (match)
            reasons.set(Number(match[1]), match[2].trim());
    }
    return reasons;
};
const explainPurchaseSuggestionWithGemini = async (items, context) => {
...
exports.explainPurchaseSuggestionWithGemini = explainPurchaseSuggestionWithGemini;
