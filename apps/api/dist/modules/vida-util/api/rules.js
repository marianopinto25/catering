...
exports.getMadurezAjuste = exports.getTemporadaAjuste = exports.findRule = exports.rulesByCategoria = exports.rulesByInsumo = exports.MADURECES = exports.TEMPORADAS = void 0;
exports.TEMPORADAS = ['Verano', 'Invierno', 'Templado'];
exports.MADURECES = ['Verde', 'Medio', 'Maduro', 'No aplica'];
const normalize = (value) => value.trim().toLowerCase();
exports.rulesByInsumo = new Map([
...
exports.rulesByCategoria = new Map([
...
const findRule = (nombre, categoria) => {
    const insumoRule = exports.rulesByInsumo.get(normalize(nombre));
    if (insumoRule)
        return { rule: insumoRule, fuente: 'regla_insumo' };
    const categoriaRule = exports.rulesByCategoria.get(normalize(categoria));
    if (categoriaRule)
        return { rule: categoriaRule, fuente: 'regla_categoria' };
    return null;
};
exports.findRule = findRule;
const getTemporadaAjuste = (rule, temporada) => {
    if (temporada === 'Verano')
        return rule.verano;
    if (temporada === 'Invierno')
        return rule.invierno;
    return rule.templado;
};
exports.getTemporadaAjuste = getTemporadaAjuste;
const getMadurezAjuste = (rule, madurez) => {
    if (!rule.aplica_madurez || madurez === 'No aplica')
        return 0;
    if (madurez === 'Verde')
        return rule.verde;
    if (madurez === 'Maduro')
...
exports.getMadurezAjuste = getMadurezAjuste;
