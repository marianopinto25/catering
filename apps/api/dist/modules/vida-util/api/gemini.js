...
exports.estimateWithGemini = void 0;
const GEMINI_ENDPOINT = 'https://generativelanguage.googleapis.com/v1beta/models';
const clampDays = (days) => Math.min(Math.max(Math.round(days), 1), 120);
const fallbackEstimate = (input) => {
...
const estimateWithGemini = async (input) => {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey)
        return fallbackEstimate(input);
    const model = process.env.GEMINI_MODEL || 'gemini-flash-latest';
    const requestEstimate = async (prompt) => fetch(`${GEMINI_ENDPOINT}/${model}:generateContent`, {
        method: 'POST',
        headers: {
...
exports.estimateWithGemini = estimateWithGemini;
